...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        @php
            if (!isset($title) || empty($title) || $title === 'Pusat Aggregator') {
                $routeName = request()->route() ? request()->route()->getName() : '';
                $routeTitles = [
                    'dashboard' => 'Dashboard',
                    'employees.index' => 'Data Pegawai',
                    'performance-reports.index' => 'Rapor Kinerja',
                    'performance-reports.show' => 'Detail Rapor Kinerja',
                    'attendance-logs.index' => 'Data Kehadiran',
                    'attendance-history.index' => 'Riwayat Kehadiran',
                    'attendance-percentage-reports.index' => 'Persentase Kehadiran',
                    'leave-approvals.index' => 'Izin Kehadiran',
                    'leave-types.index' => 'Tipe Izin',
                    'bonus-schemas.index' => 'Skema Bonus Ketepatan Waktu',
        ...
        <meta name="apple-mobile-web-app-title" content="{{ setting('app_name', 'SANS HRD') }}">
        ...
        <style>
            ...
            /* Global Dark Mode Calendar Picker Icon Styling */
    ...
    <body class="h-full bg-slate-50 dark:bg-[#09090b] text-[#09090b] dark:text-slate-50 antialiased overflow-x-hidden transition-colors duration-200">
        <div class="flex h-screen overflow-hidden">
            ...
            <div id="sidebar-backdrop" class="fixed inset-0 z-[55] bg-slate-900/20 backdrop-blur-xs hidden md:hidden transition-opacity duration-200 opacity-0"></div>
        ...
        @if(session('success'))
            <script>
                document.addEventListener('DOMContentLoaded', () => {
                    showSessionToast('Sukses!', @json(session('success')), 'success');
                });
            </script>
        @endif
        @if(session('error'))
            <script>
                document.addEventListener('DOMContentLoaded', () => {
                    showSessionToast('Perhatian!', @json(session('error')), 'error');
                });
            </script>
        @endif
