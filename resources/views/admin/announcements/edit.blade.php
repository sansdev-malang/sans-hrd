    ...
    <div class="p-6 space-y-6 w-full">
        ...
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 w-full text-left">
            <div class="flex flex-col gap-0.5">
                ...
                <h2 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-50">Ubah Pengumuman</h2>
                <p class="text-xs text-slate-500 dark:text-slate-400">Edit isi pengumuman dan kelola distribusi unit sekolah penerima.</p>
            </div>
        </header>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 text-left">
                    ...
                    <div class="lg:col-span-3 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="title" required value="{{ old('title', $announcement->title) }}" placeholder="Contoh: Pembaruan Sistem Kepegawaian Akademik" class="block w-full border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm text-sm">
                        ...
                        <div>
                            ...
                            <textarea name="content" id="content" rows="10" required class="mt-1 block w-full border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('content', $announcement->content) }}</textarea>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                @forelse($units as $unit)
                                    @php
                                        $isUnitEnabled = in_array($unit->id, $targetUnitIds);
                                        $targetInfo = collect($targetUnits)->firstWhere('school_unit_id', $unit->id);
                                        $unitAudience = $targetInfo['target_audience'] ?? 'global';
                                        
                                        $audiencesArray = explode(',', $unitAudience);
                                        $isManagementChecked = in_array('management', $audiencesArray) || in_array('global', $audiencesArray);
                                        $isTeacherChecked = in_array('teacher', $audiencesArray) || in_array('global', $audiencesArray);
                                        $isEmployeeChecked = in_array('employee', $audiencesArray) || in_array('global', $audiencesArray);
                                        $isStudentChecked = in_array('student', $audiencesArray) || in_array('global', $audiencesArray);
                                        $isParentChecked = in_array('parent', $audiencesArray) || in_array('global', $audiencesArray);
                                    @endphp
                                    @php
                                        $checkedAudiences = [];
                                        if ($isManagementChecked) $checkedAudiences[] = 'management';
                                        if ($isTeacherChecked) $checkedAudiences[] = 'teacher';
                                        if ($isEmployeeChecked) $checkedAudiences[] = 'employee';
                                        if ($isStudentChecked) $checkedAudiences[] = 'student';
                                        if ($isParentChecked) $checkedAudiences[] = 'parent';
                                    @endphp
                                    <div class="border border-slate-200 dark:border-slate-800 rounded-xl p-4 bg-slate-50/50 dark:bg-slate-900/30 text-left space-y-4" 
                                         x-data="{ 
                                             enabled: {{ $isUnitEnabled ? 'true' : 'false' }}, 
                                             audiences: {{ json_encode($checkedAudiences) }}, 
                                             toggle() { 
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="category" id="category" required class="block w-full border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm text-sm">
                                <option value="umum" {{ old('category', $announcement->category) == 'umum' ? 'selected' : '' }}>Umum / Info Sekolah</option>
                                <option value="akademik" {{ old('category', $announcement->category) == 'akademik' ? 'selected' : '' }}>Kurikulum / Akademik</option>
                                <option value="kepegawaian" {{ old('category', $announcement->category) == 'kepegawaian' ? 'selected' : '' }}>Kepegawaian / HRD</option>
                                <option value="penting" {{ old('category', $announcement->category) == 'penting' ? 'selected' : '' }}>Penting / Urgent</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="publish_date" id="publish_date" value="{{ old('publish_date', $announcement->publish_date ? $announcement->publish_date->format('Y-m-d\TH:i') : '') }}" class="block w-full border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm text-sm">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="expiry_date" id="expiry_date" value="{{ old('expiry_date', $announcement->expiry_date ? $announcement->expiry_date->format('Y-m-d\TH:i') : '') }}" class="block w-full border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm text-sm">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-slate-400 mt-1">Unggah file baru untuk mengganti lampiran sebelumnya.</p>
