                ...
                @csrf
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-2">
                            <input type="text" name="zkteco_uid" id="zkteco_uid" value="{{ old('zkteco_uid') }}" 
                            ...
                            <button type="button" id="btn-generate-uid" class="px-3 h-9 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg transition-colors">
                                Buat ID Otomatis
                            </button>
                        </div>
                    </div>
                    <div class="md:col-span-2">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                            ...
                            <label class="flex items-center space-x-2 p-2 border rounded-lg dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800">
                                <input type="checkbox" name="zkteco_device_ids[]" value="{{ $dev->id }}" class="rounded border-slate-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                <span class="text-xs font-medium text-slate-700 dark:text-slate-300">{{ $dev->name }} <span class="text-slate-400">({{ $dev->sn }})</span></span>
                            </label>
                            @endforeach
                        </div>
                        <p class="text-[10px] text-slate-500 mt-1">Centang mesin untuk mengirim data ID ZKTeco pegawai ini secara otomatis.</p>
                    </div>
                    @endif
                    <div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (btnGenerateUid && inputUid) {
                btnGenerateUid.addEventListener('click', function() {
                    const unitId = schoolUnitSelect.value;
                    if (!unitId) {
                        alert('Silakan pilih Unit Sekolah terlebih dahulu!');
                        return;
                    }
