    ...
    <div class="p-6 space-y-6" x-data="{
        ...
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 w-full text-left">
            <div class="flex flex-col gap-0.5">
                <h2 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-50">Penjadwalan Shift Pegawai
                </h2>
                <p class="text-xs text-slate-500 dark:text-slate-400">Atur penugasan dan rotasi shift kerja secara
                    kolektif per unit sekolah.</p>
            </div>
            <div class="flex items-center gap-2">
                <button type="button" @click="showCreateModal = true; loadEmployeesForUnit()"
                    class="h-9 px-4 inline-flex items-center justify-center bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 text-xs font-semibold rounded-lg border border-indigo-200 dark:border-indigo-800 transition-all cursor-pointer gap-2">
        ...
        <div
            ...
            <div
                ...
                <form method="GET" action="{{ route('employee-working-shifts.index') }}"
                    ...
                    <button type="submit"
                        class="h-8 px-4 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 text-indigo-700 dark:text-indigo-400 text-xs font-semibold rounded-lg border border-indigo-200 dark:border-indigo-800 transition-all cursor-pointer">
                        Filter Data
                    </button>
                    @if ($selectedUnitId)
                        <a href="{{ route('employee-working-shifts.index') }}"
                            class="h-8 px-4 py-2 bg-rose-50 hover:bg-rose-100 dark:bg-rose-900/30 dark:hover:bg-rose-900/50 text-rose-700 dark:text-rose-400 text-xs font-semibold rounded-lg border border-rose-200 dark:border-rose-800 transition-all cursor-pointer">Reset</a>
                    @endif
                </form>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-xs">
                    <thead
                        class="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider text-[10px]">
                        <tr>
                    ...
                    <tbody
                        class="divide-y divide-slate-100 dark:divide-slate-800/60 text-slate-700 dark:text-slate-300 font-medium">
                        @forelse($batches as $batch)
                            @if (isset($batch['type']) && $batch['type'] == 'roster')
                                <tr
                                    class="hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10 transition-colors bg-slate-50/30 dark:bg-slate-900/10">
                                    <td class="px-6 py-4 text-left">
                                        <div class="flex flex-col gap-1">
                                            <span
                                                class="font-bold text-indigo-700 dark:text-indigo-400 text-sm">{{ !empty($batch['roster_name']) ? $batch['roster_name'] : 'Roster Shift Bulanan' }}</span>
                                            <span
                                                class="inline-flex w-max px-2 py-0.5 rounded text-[10px] font-mono bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800">Bulanan</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-left">
                                        <span
                                            class="font-semibold text-slate-700 dark:text-slate-300">{{ $batch['unit_name'] }}</span>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex flex-col items-center justify-center gap-1">
                                            <span
                                                class="text-xs font-semibold text-slate-900 dark:text-slate-100">{{ \Carbon\Carbon::create($batch['year'], $batch['month'], 1)->format('M Y') }}</span>
                                            <span
                                                class="px-2 py-0.5 rounded bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400 text-[10px] font-bold uppercase tracking-wider border border-blue-200 dark:border-blue-800">Satu
                                                Bulan Penuh</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <button @click="openModal({{ json_encode($batch) }})"
                                            class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors font-bold text-xs border border-indigo-100 dark:border-indigo-800 cursor-pointer">
                            ...
                            @else
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/20 transition-colors">
                                    <td class="px-6 py-4 text-left">
                                        <div class="flex flex-col gap-1">
                                            <span
                                                class="font-bold text-slate-900 dark:text-slate-100 text-sm">{{ $batch['shift_name'] }}</span>
                                            <span
                                                class="inline-flex w-max px-2 py-0.5 rounded text-[10px] font-mono bg-slate-100 dark:bg-slate-800 text-slate-500">Kode:
                                                {{ $batch['shift_code'] }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-left">
                                        <span
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex flex-col items-center justify-center gap-1">
                                            <span
                                                class="text-xs font-semibold text-slate-900 dark:text-slate-100">{{ \Carbon\Carbon::parse($batch['start_date'])->format('d M Y') }}</span>
                                            <i data-lucide="arrow-down" class="w-3 h-3 text-slate-400"></i>
                                            @if ($batch['end_date'])
                                                <span
                                                    class="text-xs font-semibold text-slate-900 dark:text-slate-100">{{ \Carbon\Carbon::parse($batch['end_date'])->format('d M Y') }}</span>
                                            @else
                                                <span
                                                    class="px-2 py-0.5 rounded bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 text-[10px] font-bold uppercase tracking-wider border border-emerald-200 dark:border-emerald-800">Seterusnya</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div
                                        ...
                                        <div
                                            class="w-12 h-12 rounded-full bg-slate-100 dark:bg-slate-900 flex items-center justify-center text-slate-400">
                                            <i data-lucide="calendar-x" class="w-6 h-6"></i>
                                        </div>
                                        <div>
                                            <h4 class="text-sm font-bold text-slate-700 dark:text-slate-300">Belum ada
                                                jadwal shift</h4>
        ...
        <div x-show="showModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto"
            ...
            <div x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
                ...
                class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity" @click="showModal = false">
            </div>
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                ...
                <div x-show="showModal" x-transition:enter="ease-out duration-300"
                    ...
                    <div class="p-6">
                        <div
                            class="flex items-center justify-between mb-5 border-b border-slate-100 dark:border-slate-800 pb-4">
                            <div>
                                <h3 class="text-lg font-bold text-slate-900 dark:text-slate-50" id="modal-title">
                                    Daftar Pegawai Ditugaskan</h3>
                                <p class="text-xs text-slate-500 mt-1">
                                    Shift <span class="font-bold text-slate-700 dark:text-slate-300"
                                        x-text="selectedBatch?.shift_name"></span> -
                                    <span class="font-bold text-slate-700 dark:text-slate-300"
                                        x-text="selectedBatch?.unit_name"></span>
                                </p>
                            </div>
                        ...
                        <div class="mb-4" x-data="{ searchModal: '' }">
                            <div class="relative flex items-center">
                                ...
                                <input type="text" x-model="searchModal" placeholder="Cari nama pegawai..."
                                    class="w-full pr-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-indigo-500"
                                    style="padding-left: 2.25rem;">
                            </div>

                            <div class="mt-4 custom-scrollbar"
                                style="height: 350px; overflow-y: scroll; overscroll-behavior: contain; padding-right: 8px;">
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <template
                                        x-for="emp in (selectedBatch?.employees || []).filter(e => e.name.toLowerCase().includes(searchModal.toLowerCase()) || e.nip.toLowerCase().includes(searchModal.toLowerCase()))"
                                        :key="emp.id">
                                        <div
                                            class="flex items-start gap-3 p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-sm">
                                            <div
                                                class="w-8 h-8 rounded-full bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-bold text-xs uppercase shrink-0">
                                            ...
                                            <div class="flex flex-col min-w-0">
                                                ...
                                                <span class="text-[10px] font-mono text-slate-500 truncate"
                                                    x-text="`NIP/NIK: ${emp.nip}`"></span>
                                            </div>
                                        </div>
                                    </template>

                                    <div x-show="(selectedBatch?.employees || []).filter(e => e.name.toLowerCase().includes(searchModal.toLowerCase()) || e.nip.toLowerCase().includes(searchModal.toLowerCase())).length === 0"
                                        class="col-span-1 sm:col-span-2 py-8 text-center text-sm text-slate-500 italic">
                    ...
                    <div
                        class="bg-slate-50 dark:bg-slate-900/50 px-6 py-4 flex items-center justify-between border-t border-slate-200 dark:border-slate-800">
                        <span class="text-xs font-semibold text-slate-500"
                            x-text="`Total: ${selectedBatch?.employees?.length || 0} Pegawai`"></span>
                        <button type="button"
                            class="h-9 px-4 inline-flex items-center justify-center rounded-lg bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 text-xs font-semibold shadow-sm hover:bg-slate-800 dark:hover:bg-slate-200 transition-colors cursor-pointer"
        ...
        <div x-show="showAssignmentModal" style="display:none" class="fixed inset-0 z-50 overflow-hidden"
            @keydown.escape.window="showAssignmentModal=false">
            <div x-show="showAssignmentModal" x-transition:enter="ease-out duration-300"
            ...
            <div class="flex h-full items-center justify-center p-4 text-center">
                <div x-show="showAssignmentModal" x-transition:enter="ease-out duration-300"
                    ...
                    x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
                    x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                    class="relative w-full sm:max-w-4xl rounded-xl bg-white dark:bg-slate-900 shadow-2xl border border-slate-200 dark:border-slate-800 max-h-[calc(100vh-2rem)] flex flex-col overflow-hidden"
                    ...
                    <!-- Header -->
                    <div class="px-6 py-5 flex justify-between border-b border-slate-100 dark:border-slate-800 shrink-0 text-left">
                        <div>
                            <h3 class="text-lg font-bold text-slate-900 dark:text-slate-50">Tugaskan Shift Pegawai</h3>
                    ...
                    <form method="POST" action="{{ route('employee-working-shifts.store') }}"
                        class="flex flex-col flex-1 overflow-hidden text-left">
                        @csrf
                                    ...
                                    <label
                                        ...
                                        <span>
                                            ...
                                            <span x-show="isLoadingEmployees"
                                                class="text-xs text-indigo-500 ml-2 animate-pulse font-normal">Memuat
                                                data...</span>
                                        </span>
                                        <label x-show="employees.length > 0"
                                            class="flex items-center gap-1.5 cursor-pointer text-xs text-indigo-600 dark:text-indigo-400 hover:underline">
                                        ...
                                        <div
                                            ...
                                            <div class="relative flex items-center">
                                                ...
                                                <input type="text" x-model="searchQuery"
                                                    :disabled="employees.length === 0" placeholder="Cari nama atau NIK..."
                                                    style="padding-left: 2.25rem;"
                                                    class="text-xs w-full pr-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-md text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-indigo-500 disabled:opacity-50">
                                            </div>
                                        </div>

                                        <!-- List -->
                                        <div class="p-3 space-y-2 custom-scrollbar"
                                            style="height: 250px; overflow-y: scroll; overscroll-behavior: contain;">
                                            <div x-show="employees.length === 0 && !isLoadingEmployees"
                                                class="flex items-center justify-center h-full text-slate-400 italic text-xs py-8">
                                                Silakan pilih unit sekolah terlebih dahulu.
                                            </div>

                                            <div x-show="employees.length > 0 && filteredEmployees.length === 0"
                                                class="flex items-center justify-center h-full text-slate-400 italic text-xs py-8">
                                                Pegawai tidak ditemukan.
                                            </div>
                                            <template x-for="emp in employees" :key="emp.id">
                                                <label
                                                    ...
                                                    <div class="flex flex-col">
                                                        <span
                                                            class="text-xs text-slate-900 dark:text-slate-100 font-semibold leading-snug"
                                                            x-text="emp.name"></span>
                                                        <span class="text-xs text-slate-500 mt-0.5"
                                                            x-text="`NIP/NIK: ${emp.nuptk_nip_nik || '-'}`"></span>
                                                    </div>
                                                </label>
                                            </template>
                                        </div>
                        ...
                        <!-- Footer -->
                        <div class="flex gap-3 px-6 py-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-800 justify-end shrink-0">
                            <button type="button" @click="showAssignmentModal = false"
                                class="h-10 px-5 inline-flex items-center justify-center bg-white hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-semibold rounded-lg border border-slate-200 dark:border-slate-700 transition-all cursor-pointer">
                                Batal
                            </button>
                            <button type="submit"
                                class="h-10 px-6 inline-flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg shadow-sm transition-all cursor-pointer">
        ...
        <div x-show="showCreateModal" style="display: none;" class="fixed inset-0 z-50 overflow-hidden"
            ...
            <div class="flex h-full items-center justify-center p-4 text-center">
                <div x-show="showCreateModal" x-transition:enter="ease-out duration-300"
                    ...
                    x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
                    x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                    class="relative transform overflow-hidden rounded-xl bg-white dark:bg-slate-900 text-left shadow-2xl transition-all w-full sm:max-w-2xl border border-slate-200 dark:border-slate-800 max-h-[calc(100vh-2rem)] flex flex-col">
                    ...
                    <form action="{{ route('employee-working-shifts.roster') }}" method="GET" class="flex flex-col flex-1 overflow-hidden">
                            ...
                            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
                                <div>
                                    <h3 class="text-lg font-bold text-slate-900 dark:text-slate-50"
                                        id="modal-create-title">Buat Roster Bulanan Baru</h3>
                                    <p class="text-xs text-slate-500 mt-1">Pilih grup pegawai yang akan dibuatkan
                                        jadwal.</p>
                                </div>
                        ...
                        <div class="flex-1 overflow-y-auto p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <div
                                    class="border border-slate-200 dark:border-slate-800 rounded-lg bg-slate-50 dark:bg-slate-900 h-48 overflow-y-auto p-2 custom-scrollbar">
                                    <div x-show="loadingEmp" class="text-center py-4 text-xs text-slate-500">
                                        Memuat data pegawai...
                                    </div>
                                    <div x-show="!loadingEmp && empList.length === 0"
                                    ...
                                    <template x-for="emp in empList" :key="emp.id">
                                        <label x-show="empSearch === '' || emp.name.toLowerCase().includes(empSearch.toLowerCase())"
                                            ...
                                            <input type="checkbox" name="emp_ids[]" :value="emp.id"
                                                ...
                                                class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500 dark:focus:ring-offset-slate-900">
                        ...
                        <!-- Footer -->
                        <div
                            class="bg-slate-50 dark:bg-slate-900/50 px-6 py-4 flex items-center justify-end gap-3 border-t border-slate-200 dark:border-slate-800 shrink-0">
                            <button type="button"
                                class="h-9 px-4 inline-flex items-center justify-center rounded-lg bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors cursor-pointer"
                                @click="showCreateModal = false">
                                Batal
                            </button>
                            <button type="submit"
                                class="h-9 px-4 inline-flex items-center justify-center rounded-lg bg-indigo-600 text-white text-xs font-semibold shadow-sm hover:bg-indigo-700 transition-colors cursor-pointer">
                                Lanjutkan ke Grid
                            </button>
                        </div>
                    </form>
